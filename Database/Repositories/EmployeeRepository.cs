...
public class EmployeeRepository : BaseRepository<Employee>,IDisposable
{
    ...
    public override List<Employee> GetAll()
    {
        ...
        while (dataReader.Read())
        {
            Employee employee = new Employee()
            {
                ...
                DeparmentId = Convert.ToInt32(dataReader["deparmentId"])
            };
    ...
    {
        string updateQuery =
        "INSERT INTO Employees(code, name, surname, fatherName, pin, email, deparmentId) " +
        $"VALUES('{data.Code}', '{data.Name}', '{data.Surname}', '{data.FatherName}', '{data.Pin}', '{data.Email}', {data.DeparmentId} )";
    ...
    public override void Update(Employee data)
    {
        var query =
                  $"UPDATE Employees " +
                  $"SET code='{data.Code}', name='{data.Name}', surname='{data.Surname}', fatherName='{data.FatherName}', pin='{data.Pin}', email='{data.Email}', " +
                  $"WHERE id={data.Id}";
